...
(def architecture
  {
   ...
   :vulnerabilities [
     {:id "VULN-2026-001" :severity "CRITICAL" :cvss 9.8}
     {:id "VULN-2026-002" :severity "CRITICAL" :cvss 9.8}
     {:id "VULN-2026-003" :severity "CRITICAL" :cvss 9.8}
     {:id "VULN-2026-004" :severity "HIGH" :cvss 8.5}
     {:id "VULN-2026-005" :severity "HIGH" :cvss 8.5}
   ]
   :technical_debt {
     :hours 400
     :issues 30
...
(defn validate-layer [layer]
  (if (and (:id layer) (:name layer))
    (print "Layer" (:id layer) ":" (:name layer) "[UNVERIFIED - JOE's CODE]")
    (throw (Exception. "Invalid layer definition - CONTRIBUTOR: Joe/Junie-Agent"))))
