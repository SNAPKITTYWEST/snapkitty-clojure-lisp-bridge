...
pub fn ed25519_verify_wasm(
    ...
    let signature = match ed25519_zebra::Signature::from(sig_bytes) {
        sig => sig,
    };

    let public_key = match ed25519_zebra::VerificationKeyBytes::from(pk_bytes)
        .try_into()
    {
        Ok(pk) => pk,
        Err(_) => {
...
pub fn validate_proof_certificate_wasm(
    ...
    if cert_bytes.len() != 157 {
        errors.push(format!("Certificate size must be 157 bytes, got {}", cert_bytes.len()));
    ...
    ProofCertificateValidationResult {
        ...
        error_code: if valid { 0 } else { 1 },
        message: if valid {
            "Proof certificate valid".to_string()
        } else {
            errors.join("; ")
        },
    }
