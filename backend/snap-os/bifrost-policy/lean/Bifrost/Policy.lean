...
/-- A `ContextHandoff` event is valid when:
    1. The full session context (`contextCid`) is sealed in WORM_FS.
    2. The summary (`summaryCid`), if present, is also sealed in WORM_FS.
    Receiving agents load `contextCid` from WORM for lossless state recovery. -/
def validContextHandoff (e : Event) (s : State) : Prop :=
  match e with
  | Event.contextHandoff _ _ contextCid _ _ summaryCid =>
      s.wormSealed contextCid = true ∧
      (summaryCid.map (fun sc => s.wormSealed sc = true)).getD True
  | _ => False
...
def decide (e : Event) (s : State) : Bool :=
  ...
  | Event.attestation epoch rootCid _ =>
      (s.epochRoot epoch) == some rootCid
  | Event.contextHandoff _ _ contextCid _ _ summaryCid =>
      s.wormSealed contextCid &&
      summaryCid.map (fun sc => s.wormSealed sc) |>.getD true
